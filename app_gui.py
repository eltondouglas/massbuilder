# app_gui.py
# Camada de Apresentação e Controle: contém toda a classe da GUI e a lógica de eventos.
...
from data_generator import generate_from_config


class AppGeradorDados(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Gerador de Massa de Dados v3.1 (Multi-camada)")
    ...
    def _criar_widgets(self):
        ...
        ttk.Label(config_geral_frame, text="Nome do Arquivo CSV:").grid(row=0, column=0, sticky="w");
        ttk.Entry(config_geral_frame, textvariable=self.nome_arquivo_var, width=40).grid(row=0, column=1)
        ttk.Label(config_geral_frame, text="Quantidade de Linhas:").grid(row=0, column=2, sticky="w");
        ttk.Entry(config_geral_frame, textvariable=self.num_linhas_var, width=15).grid(row=0, column=3)
        ttk.Label(config_geral_frame, text="Separador:").grid(row=1, column=0, sticky="w");
        ttk.Combobox(config_geral_frame, textvariable=self.separador_var, values=list(self.separador_map.keys())).grid(
            row=1, column=1, sticky="ew")
        ttk.Label(config_geral_frame, text="Codificação:").grid(row=1, column=2, sticky="w");
        ...
        ttk.Button(sort_frame, text="Adicionar Regra de Ordenação", command=self.adicionar_regra_sort).pack(side="top",
                                                                                                            anchor="w")
    ...
    def _atualizar_comandos_e_titulos(self):
        ...
        for i, campo_dict in enumerate(self.frames_campos):
            ...
            action_frame = campo_dict['frame'].winfo_children()[-1]
            btn_subir, btn_descer, btn_remover = action_frame.winfo_children()
    ...
    def _atualizar_nomes_campos_ordenacao(self):
        ...
        for rule_dict in self.frames_sort:
            rule_dict['frame'].winfo_children()[1]['values'] = nomes_campos
    ...
    def adicionar_regra_sort(self, config=None):
        ...
        ttk.Label(rule_frame, text=f"Ordenar por:").pack(side="left");
    ...
    def iniciar_geracao(self):
        ...
        if not config: return
        try:
            # Chama a lógica de geração separada e trata possíveis erros
            generate_from_config(config)
            messagebox.showinfo("Sucesso", f"Arquivo '{config['nome_arquivo']}' gerado com sucesso!")
        except (ValueError, ImportError) as e:
            messagebox.showerror("Erro de Configuração ou Geração", str(e))
        except Exception as e:
