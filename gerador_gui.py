...
def _criar_gerador_de_campo(campo_config, total_linhas):
    """Cria um gerador que produz valores para um campo (sem alterações)."""
    valor_atual = None
...
def _criar_valor_atomico(campo):
    """Gera um único valor com base no tipo e limites (sem alterações)."""
...
def gerar_dados_gui(configuracoes):
    """Função principal que orquestra a geração de dados com base na GUI."""
    ...
    # NOVOS PARÂMETROS
    separador = configuracoes.get('separador', ',')
    codificacao = configuracoes.get('codificacao', 'utf-8')
...
class AppGeradorDados(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Gerador de Massa de Dados v2.1")
        self.geometry("950x700")  # Aumentei um pouco a altura

        # Mapa para lidar com nomes amigáveis de separadores
        self.separador_map = {
            "Vírgula (,)": ",",
            "Ponto e Vírgula (;)": ";",
            "Tab (    )": "\t",
    ...
    def _criar_widgets(self):
        ...
        # --- Frame de configurações gerais ATUALIZADO ---
        ...
        # Linha 1: Nome do Arquivo e Quantidade
        ...
        # Linha 2: Separador e Codificação (NOVO)
        ...
        # --- Frames restantes (sem alterações) ---
    ...
    def adicionar_campo(self, config=None):
        # (Função adicionar_campo não muda)
        config = config or {}
        ...
        def _atualizar_parametros(*args):
            ...
            if tipo in ['integer', 'float', 'string']:
                ...
                Tooltip(label_limites,
                        "Valor/Comprimento mínimo e máximo")
                entry_limite1.grid(
                    row=1, column=1, sticky="ew", padx=2)
                entry_limite2.grid(row=1, column=2, sticky="ew",
                                   columnspan=2, padx=2)
            elif tipo == 'datetime':
                ...
                entry_opcoes_regex.grid(row=1, column=1, sticky="ew", columnspan=4,
                                        padx=2)
            elif tipo == 'regex':
                label_limites.config(text="Padrão Regex:")
                Tooltip(label_limites,
                        "Expressão Regular para gerar os dados")
                entry_opcoes_regex.config(
                ...
                Tooltip(label_limites, "Este tipo não requer parâmetros")
    ...
    def _coletar_configuracoes(self):
        """Coleta as configurações da GUI, incluindo as novas opções."""
        try:
            config = {
            ...
            messagebox.showerror("Erro de Entrada",
                                 "Verifique se os números (linhas, limites, repetição) são válidos.")
            return None
    ...
    def carregar_configuracao(self):
        """Carrega configurações, incluindo as novas opções."""
        ...
        if filepath:
            ...
            # Lógica para carregar o separador
            separador_salvo = config.get("separador", ",")
            reverse_separador_map = {v: k for k, v in self.separador_map.items()}
            self.separador_var.set(reverse_separador_map.get(separador_salvo, separador_salvo))
    ...
    def limpar_sessao(self, confirmar=True):
        ...
        self.separador_var.set("Vírgula (,)")  # Reseta para o padrão
        self.codificacao_var.set("utf-8")  # Reseta para o padrão
